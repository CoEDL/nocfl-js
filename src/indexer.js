...
export class Indexer {
    ...
    async createIndices({ domain = undefined }) {
        ...
        walker.on("object", (object) => {
            ...
            if (!indices[domain]) indices[domain] = {};
            if (!indices[domain][className]) indices[domain][className] = {};
            if (!indices[domain][className][idPrefix]) indices[domain][className][idPrefix] = [];
    ...
    async listIndices({ domain, className }) {
        if (!domain) throw new Error(`You must provide 'domain'`);
        let prefix = `${domain}/indices`;
        if (className) prefix = `${prefix}/${className}`;
    ...
    async getIndex({ domain, className, prefix, file }) {
        if (!domain) throw new Error(`You must provide 'domain'`);
        if (!className) throw new Error(`You must provide 'className'`);
        if (!prefix && !file) throw new Error(`You must provide one of 'prefix' or 'file'`);
        let indexFile;
        if (file) {
