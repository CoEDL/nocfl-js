...
class Indexer {
      ...
      for (let className of Object.keys(indices[domain2])) {
        for (let idPrefix of Object.keys(indices[domain2][className])) {
          ...
          await this.bucket.put({
            target: indexFile,
            json: lodash.orderBy(indices[domain2][className][idPrefix], "id")
          });
        }
      }
    }
  ...
  async patchIndex({ action, domain, className, id, splay = 1 }) {
    ...
    indexFile = lodash.uniqBy(indexFile, "id");
...
const { isString, isArray, chunk } = lodash__default;
