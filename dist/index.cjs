...
class Store {
    ...
    async function putFile(params) {
      ...
      try {
        ...
        await this.bucket.put({
          ...
          json: json2,
          content: content2,
          target: nodePath__namespace.join(this.getObjectPath(), target2)
        });
  ...
  async __version({ sourceHash, target }) {
    ...
    const source = nodePath__namespace.join(this.getObjectPath(), target);
    const date = new Date().toISOString();
    const extension = nodePath__namespace.extname(target);
    const basename = nodePath__namespace.basename(target, extension);
    target = `${basename}.v${date}${extension}`;
    try {
      await this.bucket.copy({ source, target: nodePath__namespace.join(this.objectPath, target) });
